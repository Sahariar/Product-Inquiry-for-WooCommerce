...
class Product_Inquiry_Reply {
	...
	public function handle_ajax_reply() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'pi_reply_nonce' ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'Security verification failed.', 'product-inquiry' ),
				)
			);
		}
		...
		if ( ! $inquiry_id || 'product_inquiry' !== get_post_type( $inquiry_id ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'Invalid inquiry.', 'product-inquiry' ),
				)
			);
		}
		...
		if ( ! current_user_can( 'edit_post', $inquiry_id ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'You do not have permission to reply to this inquiry.', 'product-inquiry' ),
				)
			);
		}
		...
		if ( empty( $reply_message ) || strlen( $reply_message ) < 10 ) {
			wp_send_json_error(
				array(
					'message' => __( 'Please enter a reply message with at least 10 characters.', 'product-inquiry' ),
				)
			);
		}
		...
		if ( ! $inquiry_data ) {
			wp_send_json_error(
				array(
					'message' => __( 'Failed to retrieve inquiry data.', 'product-inquiry' ),
				)
			);
		}
		...
		if ( ! $email_sent ) {
			wp_send_json_error(
				array(
					'message' => __( 'Failed to send reply email. Please check your email configuration.', 'product-inquiry' ),
				)
			);
		}
		...
		update_post_meta( $inquiry_id, '_pi_status', 'replied' );
	...
	private function get_inquiry_data( $inquiry_id ) {
		...
		$product_title = $product ? $product->get_name() : __( '(Product Deleted)', 'product-inquiry' );
	...
	private function send_reply_email( $inquiry_id, $inquiry_data, $reply_message ) {
		...
		$subject = sprintf(
			...
			__( 'Response to your inquiry about: %s', 'product-inquiry' ),
	...
	private function build_reply_email_body( $inquiry_data, $reply_message, $current_user ) {
		$admin_name = ! empty( $current_user->display_name ) ? $current_user->display_name : __( 'Store Admin', 'product-inquiry' );

		$body_parts = array(
			sprintf(
				/* translators: %s: Customer name */
				__( 'Hello %s,', 'product-inquiry' ),
				$inquiry_data['customer_name']
			),
			'',
			__( 'Thank you for your inquiry. Here is our response:', 'product-inquiry' ),
			'',
			'--- ' . __( 'Our Response', 'product-inquiry' ) . ' ---',
			'',
			$reply_message,
			'',
			'--- ' . __( 'Your Original Message', 'product-inquiry' ) . ' ---',
			'',
			sprintf(
				/* translators: %s: Product title */
				__( 'Product: %s', 'product-inquiry' ),
				$inquiry_data['product_title']
			),
		);
		...
		if ( ! empty( $inquiry_data['product_url'] ) ) {
			$body_parts[] = sprintf(
				...
				__( 'Product Link: %s', 'product-inquiry' ),
		...
		$body_parts = array_merge(
			...
			array(
				...
				sprintf(
					/* translators: %s: Date */
					__( 'Date: %s', 'product-inquiry' ),
					$inquiry_data['date']
				),
				'',
				__( 'Your Message:', 'product-inquiry' ),
				$inquiry_data['message'],
				'',
				'--- ' . __( 'Contact Information', 'product-inquiry' ) . ' ---',
				'',
				sprintf(
					/* translators: %s: Admin name */
					__( 'Best regards,', 'product-inquiry' ) . "\n%s",
					$admin_name
				),
				get_bloginfo( 'name' ),
	...
	private function log_reply( $inquiry_id, $reply_message ) {
		...
		$replies = get_post_meta( $inquiry_id, '_pi_replies', true );
		...
		// Update meta
		update_post_meta( $inquiry_id, '_pi_replies', $replies );
	}
	...
	private function format_last_reply( $inquiry_id ) {
		...
		return sprintf(
			...
			__( 'Last reply by %1$s on %2$s', 'product-inquiry' ),
	...
	public function render_replies_metabox( $post ) {
		$replies = get_post_meta( $post->ID, '_pi_replies', true );

		if ( ! is_array( $replies ) || empty( $replies ) ) {
			echo '<p>' . esc_html__( 'No replies sent yet.', 'product-inquiry' ) . '</p>';
			return;
		}
				...
				<div class="pi-reply-header">
					<strong>
						...
						printf(
							...
							esc_html__( 'Reply #%1$d by %2$s', 'product-inquiry' ),
							$reply_number,
							esc_html( $reply['user_name'] )
						);
						?>
					</strong>
	...
	public function render_reply_form_metabox( $post ) {
		$customer_email = get_post_meta( $post->ID, '_pi_email', true );
		$customer_name  = get_post_meta( $post->ID, '_pi_name', true );

		wp_nonce_field( 'pi_reply_nonce', 'pi_reply_nonce_field' );
		?>
		<div class="pi-reply-form">
			<p>
				<strong><?php esc_html_e( 'Send Reply To:', 'product-inquiry' ); ?></strong><br>
				<?php
				printf(
					'%s (%s)',
			...
			<p>
				<label for="pi_reply_message">
					<?php esc_html_e( 'Your Reply:', 'product-inquiry' ); ?>
				</label>
				<textarea 
					id="pi_reply_message" 
					...
					rows="8" 
					class="widefat"
					placeholder="<?php esc_attr_e( 'Type your reply here...', 'product-inquiry' ); ?>"
				></textarea>
			</p>

			<p class="pi-reply-actions">
				<button type="button" id="pi_send_reply_btn" class="button button-primary button-large">
					<?php esc_html_e( 'Send Reply', 'product-inquiry' ); ?>
				</button>
	...
	public function add_reply_metaboxes() {
		...
		add_meta_box(
			'pi_replies_history',
			__( 'Reply History', 'product-inquiry' ),
		...
		wp_localize_script(
			...
			array(
				...
				'messages' => array(
					...
					'empty'        => __( 'Please enter a reply message.', 'product-inquiry' ),
					'confirm_send' => __( 'Are you sure you want to send this reply?', 'product-inquiry' ),
				),
			)
		);
	}
